      ...
      @endif
        ...
        <div class="form-group">
            <select class="selectpicker form-control" name="node_id" >
              ...
              <option value="" disabled {!! App::make('\App\Topic')->present()->haveDefaultNode($node, null) ?: 'selected'; !!}>{{ lang('Pick a node') }}</option>

              @foreach ($nodes['top'] as $top_node)
                <optgroup label="{{{ $top_node->name }}}">
                  @foreach ($nodes['second'][$top_node->id] as $snode)
                    <option value="{{ $snode->id }}" {!! App::make('\App\Topic')->present()->haveDefaultNode($node, $snode) ? 'selected' : ''; !!} >{{ $snode->name }}</option>
                  @endforeach
                </optgroup>
              @endforeach
            </select>
