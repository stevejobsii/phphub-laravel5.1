    ...
    <div class="infos panel-heading">
      ...
      <div class="pull-right avatar_large">
        <a href="{{ route('users.show', $topic->user->id) }}">
          <img src="{{ $topic->user->present()->avatar }}" style="width:65px; height:65px;" class="img-thumbnail avatar" />
        </a>
      </div>
      ...
      <div class="votes">
        ...
        <a data-method="post" href="javascript:void(0);" data-url="{!! route('topics.upvote', $topic->id) !!}" title="{{ lang('Up Vote') }}" id="up-vote" class="vote {!! Auth::user() && $topic->votes()->ByWhom(Auth::id())->WithType('upvote')->count() ? 'active' :''; !!}">
            <li class="fa fa-chevron-up"></li> {{ $topic->vote_count }}
        </a>
         &nbsp;
        <a data-method="post" href="javascript:void(0);" data-url="{!! route('topics.downvote', $topic->id) !!}" title="{{ lang('Down Vote') }}" id="down-vote" class="vote {!! Auth::user() && $topic->votes()->ByWhom(Auth::id())->WithType('downvote')->count() ? 'active' :''; !!}">
